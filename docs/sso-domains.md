...
2. **Remote** — Tailscale only by default. Do **not** open 80/443 (or app ports) on the residential router to the internet for this profile.
3. **DNS for AdGuard** — Point household devices at AdGuard’s DNS listeners on LAN (and optionally via Tailscale IPs). Do not expose AdGuard’s DNS ports to the open internet.
...
`headscale` is in the YunoHost catalog for a self-hosted coordination server. Tailscale clients still install at host/OS level. Treat Headscale as optional control-plane work after the v1 app set is stable — not a day-0 requirement.
