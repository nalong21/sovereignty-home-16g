...
| **Jitsi** | Real-time A/V wants sustained CPU/RAM and often tempts public exposure. Conflicts with “no public exposure by default” unless carefully mesh-only (Headscale) — still a support burden on 16GB next to Immich. |
...
Document: expected RAM/CPU, whether it stays mesh-only (Headscale), backup impact, and what you will remove or resize (e.g. Immich ML schedule) to make room.
